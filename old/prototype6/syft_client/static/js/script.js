    ...
    availablePlugins.forEach(plugin => {
        ...
        div.innerHTML = `
            ...
            <div class="plugin-controls">
                <input type="number" value="${plugin.default_schedule}" placeholder="Schedule (ms)">
                <button onclick="launchPlugin('${plugin.name}', this.previousElementSibling.value)">
    ...
    axios.get('/running')
        .then(response => {
            ...
            Object.entries(running).forEach(([plugin, data]) => {
                ...
                div.innerHTML = `
                    <span>${plugin}</span>
                    <span>Running: ${data.is_running}, Time: ${data.run_time.toFixed(2)}s, Schedule: ${data.schedule}ms</span>
...
function launchPlugin(plugin, schedule) {
    axios.post('/launch', { plugin_name: plugin, schedule: parseInt(schedule) })
        .then(response => {
...
function killPlugin(plugin) {
    axios.post('/kill', { plugin_name: plugin })
        .then(response => {
            console.log(response.data.message);
            loadRunningPlugins();
        })
...
function viewSharedState() {
    ...
    axios.get('/state')
        .then(response => {
            ...
            document.getElementById('stateDisplay').textContent = JSON.stringify(response.data, null, 2);
        ...
        .catch(error => {
            ...
            document.getElementById('stateDisplay').textContent = 'Error fetching shared state';
        });
}
...
function updateSharedStateField(key) {
    ...
    axios.post('/state/update', { key, value })
        .then(response => {
        ...
        .catch(error => {
            ...
            alert(`Error updating ${key}: ${error.response ? error.response.data.error : error.message}`);
...
function addDatasite() {
    ...
    axios.post('/datasites', { name: name })
        .then(response => {
            alert(response.data.message);
            loadDatasites();
