    ...
    for datasite in syft_datasites:
        ...
        if not os.path.exists(syftbox_datasite_path):
            try:
                ...
                logger.info(
                    f"Created datasite folder in SyftBox: {syftbox_datasite_path}"
                )

                # Copy the public key
                syft_public_key_path = os.path.join(
                    syft_datasite_path, "public_key.pem"
                )
                syftbox_public_key_path = os.path.join(
                    syftbox_datasite_path, "public_key.pem"
                )
                ...
                if os.path.exists(syft_public_key_path):
                    ...
                    syftperm_path = os.path.join(
                        syftbox_datasite_path, "public_key.pem.syftperm"
                    )
                    perm_content = {"READ": ["EVERYONE", datasite]}
                ...
                logger.error(
                    f"Failed to synchronize datasite {datasite}. Error: {str(e)}"
                )
        else:
            ...
            if not os.path.exists(syftperm_path):
                try:
                    ...
                    with open(syftperm_path, "w") as perm_file:
                        json.dump(perm_content, perm_file, indent=2)
                    logger.info(
                        f"Created missing .syftperm file for existing datasite: {datasite}"
                    )
                except Exception as e:
                    logger.error(
                        f"Failed to create .syftperm file for existing datasite {datasite}. Error: {str(e)}"
                    )

    logger.info(
        "Datasite folder synchronization and permission file creation completed"
    )
